...
namespace SRPSimulator
{
    ...
    public partial class UnitControl : UserControl
    {
        ...
        public void StateChanged()
        {
            ...
            PUnitConfigEmbedded unitConfig = unit.Config as PUnitConfigEmbedded;

            // Joints positions;
            BeamConnectionRodJoint.X = BeamAxisCenter.X + Math.Cos(PhysicsStuff.DegreeToRadians(-unit.Alpha)) * Scale * -unitConfig.SizeC;
            BeamConnectionRodJoint.Y = BeamAxisCenter.Y + Math.Sin(PhysicsStuff.DegreeToRadians(-unit.Alpha)) * Scale * unitConfig.SizeC;

            CrankConnectionRodJoint.X = CrankAxisCenter.X + Math.Sin(PhysicsStuff.DegreeToRadians(unit.Fi)) * Scale * unitConfig.SizeR;
            CrankConnectionRodJoint.Y = CrankAxisCenter.Y + Math.Cos(PhysicsStuff.DegreeToRadians(unit.Fi)) * Scale * unitConfig.SizeR;

            connectionRod.SetPosition(unit.CRAngle, CrankConnectionRodJoint, BeamConnectionRodJoint);
            rod.SetState(unit.RodX * PhysicsStuff.KILO);
        }
        ...
        public void SizesChanged()
        {
            ...
            { 
                return Math.Ceiling(value * PhysicsStuff.MILLI) * PhysicsStuff.KILO;
            }
